...
import { StatCard, AlertBanner } from '../components/SharedComponents';
...
const STA_BADGE  = { Active:'danger', Pending:'warning', Verified:'info', Responded:'purple', Resolved:'success' };
...
export default function Dashboard() {
  ...
  const monthLabels = Array.from({length:7},(_,i)=>{
    ...
    return { month:MONTHS[d.getMonth()], idx:d.getMonth(), fullYear:d.getFullYear() };
  });
      ...
      <div className="dashboard-main-row">
        <div className="card">
          <div className="section-title">
            ...
            <span className="badge badge-danger" style={{marginLeft:'auto'}}>{activeInc.length} Active</span>
          </div>
            ...
            {activeInc.map(inc => (
              <div key={inc.id} className="incident-row">
                ...
                <div className="inc-right">
                  ...
                  <span className={`badge badge-${STA_BADGE[inc.status]}`}>{inc.status}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="card">
            <div className="weather-card-top">
              <div>
                <div className="section-title" style={{marginBottom:0}}>
                  ...
                  {weather.loading && <i className="fa-solid fa-spinner fa-spin" style={{fontSize:12,marginLeft:8}}></i>}
                </div>
                <div style={{fontSize:11,color:'var(--text-muted)',marginTop:4}}>Barangay Kauswagan, CDO</div>
              </div>
            ...
            <div className="weather-main">
              ...
              <div className="weather-cond">
                <i className={`fa-solid ${weather.icon || 'fa-cloud'} weather-cond-icon`}></i>
                <span>{weather.condition}</span>
              </div>
            </div>
            {showWeather && (
              <div className="weather-detail">
                <div className="weather-stat"><i className="fa-solid fa-droplet"></i><span>Humidity</span><strong>{weather.humidity}%</strong></div>
                <div className="weather-stat"><i className="fa-solid fa-wind"></i><span>Wind Speed</span><strong>{weather.windSpeed} km/h</strong></div>
                <div className="weather-stat"><i className="fa-solid fa-cloud-showers-heavy"></i><span>Rainfall (1h)</span><strong>{weather.rainfall1h} mm</strong></div>
                <div className="weather-risk">
                  <i className="fa-solid fa-circle-exclamation"></i>
                  Risk Level: <strong style={{color:'var(--accent-red)'}}>{weather.riskLevel}</strong>
                </div>
                {weather.error && <div style={{fontSize:10,color:'var(--text-muted)',marginTop:4}}>{weather.error}</div>}
              </div>
            )}
          </div>
                  ...
                  <div key={z.zone} className="zone-row">
                    ...
                    <div className="zone-bar-wrap">
                      <div className={`zone-bar zone-${z.riskLevel.toLowerCase()}`}
                           style={{width:`${Math.max(Math.min((liveCount/Math.max(incidents.length,1))*200,100),8)}%`}}>
                      </div>
                    </div>
            ...
            {trendData.map(item=>(
              <div key={item.month} className="trend-bar-col">
                <div className="trend-bar-wrap">
                  <div className="trend-bar-fill" style={{height:`${item.count===0?2:(item.count/maxTrend)*100}%`}}></div>
                </div>
