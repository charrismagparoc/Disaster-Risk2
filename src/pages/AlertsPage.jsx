...
import { AlertBanner, ConfirmModal } from '../components/SharedComponents';
...
export default function AlertsPage() {
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setShowModal(false);
        setSent(false);
        setForm({ level:'Advisory', zone:'All Zones', message:'', channel:'Web' });
      }, 1600);
      setSending(false);
    }, 700);
  };
  ...
  const openQuick = (q) => {
    setForm({ level:q.level, zone:q.zone, message:q.msg, channel:'Web' });
    setShowModal(true);
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div><div className="page-title">Alert System</div><div className="page-subtitle">Broadcast emergency alerts across all barangay zones — saved in real-time</div></div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:10}}>
        ...
        {alerts.map(alert=>(
          <div key={alert.id} style={{position:'relative'}}>
            ...
            <button style={{position:'absolute',top:10,right:10,background:'rgba(230,57,70,.12)',color:'var(--accent-red)',border:'1px solid rgba(230,57,70,.25)',borderRadius:6,padding:'5px 10px',fontSize:12,cursor:'pointer',display:'flex',alignItems:'center',gap:5}} onClick={()=>setDeleteId(alert.id)}>
      ...
      {deleteId&&<ConfirmModal title="Delete Alert" message="Remove this alert from history?" onConfirm={()=>{deleteAlert(deleteId);setDeleteId(null);}} onCancel={()=>setDeleteId(null)}/>}
            ...
            {sent?(
              ...
              <div className="form-grid">
                ...
                <div className="form-group full"><label>Alert Message</label>
                  <textarea className="form-control" rows={4} value={form.message} onChange={e=>setForm({...form,message:e.target.value})} placeholder="e.g. FLOOD WARNING: Water level critically high..."></textarea>
                </div>
