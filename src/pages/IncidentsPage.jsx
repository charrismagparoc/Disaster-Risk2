...
export default function IncidentsPage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (isEditing && selected) updateIncident(selected.id, form);
      else addIncident(form);
      setShowModal(false);
  ...
  return (
    <div>
      <div className="page-header">
        <div><div className="page-title">Incident Management</div><div className="page-subtitle">Track, verify, and respond to reported incidents</div></div>
        <button className="btn btn-primary" onClick={openNew}><i className="fa-solid fa-plus"></i> Report Incident</button>
      </div>

      <div className="summary-pills">
        {statuses.slice(1).map(s => (
          <div key={s} className={`summary-pill badge badge-${statusColor[s]}`} onClick={()=>setFilterStatus(filterStatus===s?'All':s)}>
        ...
        <div className="table-container">
          <table>
            ...
            <tbody>
              {filtered.map(inc=>(
                <tr key={inc.id}>
                  <td><span className="mono">{(inc.id||'').slice(0,8)}</span></td>
                  <td>
                  ...
                  <td>{inc.reporter}</td>
                  <td><span className={`badge badge-${sevColor[inc.severity]}`}>{inc.severity}</span></td>
                  ...
                  <td>
                    <div style={{display:'flex',gap:5}}>
                      <button className="btn btn-secondary btn-sm" onClick={()=>openView(inc)}><i className="fa-solid fa-eye"></i></button>
                      <button className="btn btn-outline btn-sm" onClick={()=>openEdit(inc)}><i className="fa-solid fa-pen"></i></button>
                      {inc.status!=='Resolved'&&<button className="btn btn-success btn-sm" onClick={()=>updateStatus(inc.id,'Resolved')}><i className="fa-solid fa-check"></i></button>}
                      <button className="btn btn-sm" style={{background:'rgba(230,57,70,.12)',color:'var(--accent-red)',border:'1px solid rgba(230,57,70,.3)'}} onClick={()=>setDeleteId(inc.id)}><i className="fa-solid fa-trash"></i></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {deleteId && <ConfirmModal title="Delete Incident" message="Are you sure you want to delete this incident record?" onConfirm={()=>{deleteIncident(deleteId);setDeleteId(null);setShowModal(false);}} onCancel={()=>setDeleteId(null)}/>}
            ...
            {selected && !isEditing ? (
              <div>
                <div className="detail-grid">
                  ...
                  <div className="detail-item"><span>Severity</span><span className={`badge badge-${sevColor[selected.severity]}`}>{selected.severity}</span></div>
              ...
              <div className="form-grid">
                ...
                <div className="form-group"><label>Reported By</label><input className="form-control" placeholder="Full name..." value={form.reporter} onChange={e=>setForm({...form,reporter:e.target.value})}/></div>
                <div className="form-group full"><label>Description</label><textarea className="form-control" rows={3} value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder="Describe the situation..."></textarea></div>
